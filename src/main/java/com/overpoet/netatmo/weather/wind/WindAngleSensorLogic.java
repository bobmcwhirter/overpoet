...
public class WindAngleSensorLogic extends AbstractJSONSensorLogic<Bearing,Integer> {

    private final static JsonPath PATH = JsonPath.compile("$.body.devices[0].modules[?(@.type == 'NAModule2')].dashboard_data.WindAngle");

    public WindAngleSensorLogic() {
    ...
    public static Sensor<?> of(Key key, LogicRegistry registry) {
        return new BearingSensor(key.append("wind-angle"), BearingMetadata.DEFAULT, registry.register(new WindAngleSensorLogic()));
    }
