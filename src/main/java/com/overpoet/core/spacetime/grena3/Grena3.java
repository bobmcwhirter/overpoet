...
public final class Grena3 {
    ...
    private static double calcT(ZonedDateTime date) {
        ...
        int m = utc.get(ChronoField.MONTH_OF_YEAR) + 1;
        int y = utc.get(ChronoField.YEAR);
        final int d = utc.get(ChronoField.DAY_OF_MONTH);
        final double h = utc.get(ChronoField.HOUR_OF_DAY) +
                utc.get(ChronoField.MINUTE_OF_HOUR) / 60d +
                utc.get(ChronoField.SECOND_OF_MINUTE) / (60d * 60);
